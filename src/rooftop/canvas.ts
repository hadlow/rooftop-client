...
export class Canvas
{
	...
	public updateGroups(id: string, group: any)
	{
		if(id == this.mirror.getId())
		{
			return this.mirror.joinGroup(group.id, group.color);
		}

		for(let client of this.clients)
		{
			if(client.getId() == id)
				client.joinGroup(group.id, group.color);
		}
	}

	public leftGroup(id: string)
	{
		if(id == this.mirror.getId())
			return this.mirror.leaveGroup();
	...
	private onMouseUp(): void
	{
		this.canvas.onmouseup = () =>
		{
			...
			if(this.mouseOnMirror)
			{
				...
				{
					let mirrorGroupId = (this.mirror.getGroup() ? this.mirror.getGroup().id : '');
					let nearestGroupId = (this.mirror.getNearest().getGroup() ? this.mirror.getNearest().getGroup().id : '');

					// If we're not already part of a group, or joining another group
					if(this.mirror.getGroup() == undefined || mirrorGroupId != nearestGroupId)
					{
						...
						if(group == undefined)
						{
							...
							this.emit('group.join', {"group": {"id": group.id, "color": group.color}});
							console.log('group joined: ' + group.id);
						}
					}
				} else {
					if(this.mirror.getGroup() != undefined)
					{
	...
	{
		for(let client of this.clients)
			client.draw(this.originX, this.originY, this.width, this.height);
	}

	private drawMirror()
	{
		if(this.mirror)
			this.mirror.draw(this.originX, this.originY, this.width, this.height);
	}
	...
	private highlightNearest(x: number, y: number)
	{
		...
		{
			if(otherClient.getId() != nearest.getId())
				otherClient.setFar();
		}
		...
		if(nearest)
		{
			if(nearest.distance(x, y) < 150)
			{
				...
				for(let client of this.clients)
				{
					if(client.getGroup() != undefined && nearest.getGroup() != undefined)
					{
						if(client.getGroup().id == nearest.getGroup().id)
							client.setNear();
					}
				}
			} else {
